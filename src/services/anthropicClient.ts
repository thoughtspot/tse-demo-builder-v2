/**
 * Client-side functions for interacting with Anthropic API routes
 * These functions call server-side API routes to securely handle API keys
 *
 * For direct API usage, use the AnthropicClient class from anthropicService.ts
 */
...
export const classifyQuestion = async (
  ...
  try {
    ...
    const response = await fetch("/api/anthropic/classify", {
      method: "POST",
      headers: {
...
/**
 * Checks if Anthropic is available (always returns true since we have fallback)
 */
